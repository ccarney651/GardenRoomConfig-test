...
function sendEnquiryEmail() {
  ...
  const specs = [
    ...
    `Foundation:  ${FOUNDATION[state.foundation]?.label}`,
    `Roof Style:  ${ROOF_STYLE[state.roof]?.label}`,
    `Roof Finish: ${ROOF_FINISH[state.roofFinish]?.label}`,
    `Cladding:    ${CLADDING[state.cladding]?.label}`,
    `Doors:       ${state.openings.filter(o=>o.type==='door').map(o=>DOOR[o.style]?.label??o.style).join(', ')||'None'}`,
    `Windows:     ${state.openings.filter(o=>o.type==='window').map(o=>WINDOW_STYLE[o.style]?.label??o.style).join(', ')||'None'}`,
  ].join('\n');
